...
def enumerate_accepted_programs(
    model: FiniteControlModel, max_actions: int
    ...
    def walk(syntax_state: str, names: tuple[str, ...], bits: str) -> None:
        ...
        for action in model.actions_by_syntax[syntax_state]:
            ...
            if action.is_halt:
                accepted.append(AcceptedProgram(next_names, next_bits))
            else:
                assert action.successor is not None
                walk(action.successor, next_names, next_bits)

    walk(model.start_syntax, (), "")
    return accepted
...
def program_output(model: FiniteControlModel, program: AcceptedProgram) -> Array:
    require_valid_model(model)
...
def depth_contribution_by_enumeration(
    model: FiniteControlModel, action_count: int
) -> Array:
    contribution = np.zeros((model.output_dim, model.output_dim), dtype=complex)
    for program in enumerate_accepted_programs(model, action_count):
        if program.action_count == action_count:
            contribution += program.weight * program_output(model, program)
    return contribution
...
def apply_halt_map(model: FiniteControlModel, state: dict[BlockKey, Array]) -> Array:
    ...
    for (syntax_state, source), operator in state.items():
        for action in model.actions_by_syntax[syntax_state]:
            ...
            output += weight * apply_kraus(kraus, operator)
    return output


def depth_contribution_by_maps(model: FiniteControlModel, action_count: int) -> Array:
    require_valid_model(model)
    if action_count < 1:
...
def truncated_semidensity(model: FiniteControlModel, max_transient_steps: int) -> Array:
    require_valid_model(model)
...
def transient_matrix(model: FiniteControlModel) -> Array:
    require_valid_model(model)
    keys, slices = block_slices(model)
...
def halt_matrix(model: FiniteControlModel) -> Array:
    require_valid_model(model)
    keys, slices = block_slices(model)
    total_dim = sum(model.control_dims[key[1]] ** 2 for key in keys)
