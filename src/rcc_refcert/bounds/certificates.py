...
from .contracts import Task, canonical_hash, text
...
def _snapshot(value):
    ...
    if isinstance(value, np.ndarray):
        return {
            ...
            "values": [_snapshot(x) for x in value.flat],
        }
...
def with_reference_certificate(
    ...
    *,
    ...
    model and certificate inputs. The matrix verification retains its existing
    ...
    result["model"]["reference_domination"] = {
        ...
        "evidence": {
            ...
            "model_sha256": canonical_hash(_snapshot(model)),
            "certificate_sha256": canonical_hash(_snapshot(certificate)),
